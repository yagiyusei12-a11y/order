...
$remote = "set -e; cd $remotePath; git pull; bash deploy/vps/install-harunoyukoto-php.sh; npm ci; npx prisma migrate deploy; npx prisma generate; npm run build; sudo systemctl restart $service; sleep 2; curl -sS http://127.0.0.1:3000/health"
if ($daikoDeploy) {
