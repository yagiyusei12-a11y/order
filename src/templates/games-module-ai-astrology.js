(function () {
  const THEMES = ["恋愛", "仕事・キャリア", "金運", "総合運"];
  ...
  function renderAiResult(root, aiResult) {
    let html = '<div class="af-result"><h2>' + esc(aiResult.title || "占星術鑑定") + "</h2>";
  ...
  window.__gameModules["ai-astrology"] = {
    mount(ctx) {
      ...
      function renderForm() {
        ...
        root.innerHTML =
          '<div class="af-wrap">' +
          '<p class="af-hint">西洋占星術でホロスコープを読み解く本格鑑定。<br>出生時刻・出生地があるとより精密になります。</p>' +
          '<label class="af-field"><span class="af-label">生年月日</span><input id="afBirth" type="date" /></label>' +
          '<label class="af-field"><span class="af-label">出生時刻（任意）</span><input id="afTime" type="time" /></label>' +
          '<label class="af-field"><span class="af-label">出生地（任意）</span><input id="afPlace" type="text" maxlength="60" placeholder="例: 滋賀県長浜市" /></label>' +
          '<label class="af-field"><span class="af-label">鑑定テーマ</span><select id="afTheme">' +
          opts +
          "</select></label>" +
          '<label class="af-field"><span class="af-label">相談（任意）</span><textarea id="afQuestion" maxlength="200" placeholder="例: 今のパートナーとの相性は？"></textarea></label>' +
          "</div>";
      }
      ...
      btn.onclick = async () => {
        ...
        if (phase === "form") {
          ...
          const birthPlace = (document.getElementById("afPlace")?.value || "").trim();
          const theme = document.getElementById("afTheme")?.value || "";
          const question = (document.getElementById("afQuestion")?.value || "").trim();
          if (!birthDate) {
            showErr("生年月日を入力してください");
            return;
          }
          btn.disabled = true;
          btn.textContent = "星を読み解き中…";
          root.innerHTML = '<p class="af-hint">ホロスコープを展開しています…</p>';
          try {
